...
void mostrarUsuario(sqlite3 *db, const char *nombreUsuario) {
    ...
    snprintf(sql, sizeof(sql), "SELECT nombre, apellidos, dni, direccion, email, telefono, es_Admin FROM Usuario WHERE nombre = ?;");
...
void editarUsuario(sqlite3 *db, const char *nombreUsuario) {
    ...
    do {
        ...
        if (opcion == '0') break;  // Si se elige '0', salir del bucle

        // Dependiendo de la opción seleccionada, recuperar el valor actual de la base de datos
        switch(opcion) {
            case '1': // Nombre
                snprintf(sql, sizeof(sql), "SELECT nombre FROM Usuario WHERE nombre = ?;");
                break;
            case '2': // Dirección
                snprintf(sql, sizeof(sql), "SELECT direccion FROM Usuario WHERE nombre = ?;");
                break;
            case '3': // Email
                snprintf(sql, sizeof(sql), "SELECT email FROM Usuario WHERE nombre = ?;");
                break;
            case '4': // Teléfono
                snprintf(sql, sizeof(sql), "SELECT telefono FROM Usuario WHERE nombre = ?;");
                break;
            case '5': // Contraseña
                snprintf(sql, sizeof(sql), "SELECT contrasena FROM Usuario WHERE nombre = ?;");
                break;
            default:
        ...
        switch(opcion) {
            case '1': snprintf(sql, sizeof(sql), "UPDATE Usuario SET nombre = ? WHERE nombre = ?;"); break;
            case '2': snprintf(sql, sizeof(sql), "UPDATE Usuario SET direccion = ? WHERE nombre = ?;"); break;
            case '3': snprintf(sql, sizeof(sql), "UPDATE Usuario SET email = ? WHERE nombre = ?;"); break;
            case '4': snprintf(sql, sizeof(sql), "UPDATE Usuario SET telefono = ? WHERE nombre = ?;"); break;
            case '5': snprintf(sql, sizeof(sql), "UPDATE Usuario SET contrasena = ? WHERE nombre = ?;"); break;
        }
